...
class RulesService extends BaseService{
    findRulesById (req, res, next) {
        if(req.rule && req.rule.ruleType == 'Decision'){
            ...
            return this.db.any( 'SELECT enrolment_eligibility_check_rule as rules from program inner join program_enrolment on program.id=program_enrolment.program_id where program_enrolment.uuid = $1', [req.rule.formUuid]);
                }
    }
}
